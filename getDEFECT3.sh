...
#trimer part 1 atom 41 :  0.0110128200         0.028503999         0.277350008
#timer part 1 atom 37 : 0.0110128200         0.225538999         0.474384993
...
alatt=`sed -n '3,3p' XDATCAR  | awk '{printf "%1.6f", $1}'`
limitlatt=`echo "scale=6; $alatt*$limit " | bc -l`
#remove lattice parameter lines from trajectory file
sed '1,7d' XDATCAR > xdat
#Separate the trajectory for the defect atom
for i in {1..200}; do var=`echo "scale=6; $i*66-1" | bc -l` ; sed -n ''$var','$var'p' xdat ; done | column -t > def_traj_dir_1
#Separate the trajectory for the defect atom2
for i in {1..200}; do var=`echo "scale=6; $i*66-24" | bc -l` ; sed -n ''$var','$var'p' xdat ; done | column -t > def_traj_dir_2
#Separate the trajectory for the defect atom3
for i in {1..200}; do var=`echo "scale=6;$i*66-12" | bc -l` ; sed -n ''$var','$var'p' xdat ; done | column -t > def_traj_dir_3
...
for j in {1..9}; do 
  for i in {1..200}; do
    ...
    if (( $(echo "scale=6 ;$v1 > $limitlatt" |bc -l) )); then
      v2=`echo "scale=6 ; $v1 - $alatt" | bc -l` && sed -i 's/'$v1'/'$v2'/g' tmp
    fi
    echo $v1  $v2
  done
done
